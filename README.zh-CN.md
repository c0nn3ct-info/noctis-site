...
Noctis 是一款免费的浏览器扩展，它通过基于 sing-box 的本地助手，把 Chrome 的流量路由到 VLESS、VMess、Trojan、Shadowsocks、Hysteria2、Reality 等代理服务器。无需系统级 VPN，也没有单独的客户端窗口——代理始终在浏览器内部进行。
...
- **实时日志流** — sing-box 的 stdout 和 stderr 直接流入扩展。无需离开浏览器即可诊断连接问题。
...
Noctis 支持 sing-box 提供的所有传输方式：VLESS（包括 VLESS Reality）、VMess、Trojan、Shadowsocks、Hysteria2、TUIC、WireGuard、AnyTLS 和 ShadowTLS。来自 V2Ray、Xray 和 3X-UI 面板的配置可直接使用——粘贴分享链接或订阅 URL，扩展会自动把它转换成 sing-box 的出站。

## 🧩 工作原理

浏览器自己无法运行 sing-box 引擎。三个部分把工作分摊到沙箱边界两侧——而跨越边界的那个箭头是消息流动的唯一通道。
...
sing-box 是运行在 Noctis 底层的开源代理引擎。它兼容 V2Ray 和 Xray 配置，开箱即支持 VLESS、VMess、Trojan、Shadowsocks、Hysteria2、TUIC、Reality、AnyTLS、ShadowTLS 和 WireGuard。Noctis 捆绑了一个小型原生助手，在你的机器上管理 sing-box，因此浏览器扩展只需发送路由决策——而不会传输原始流量。

## 📥 安装

Noctis 扩展需要在你的机器上运行一个小型原生助手。该助手负责管理 sing-box，也就是真正执行代理的引擎。

### 开始之前

- 基于 Chromium 的浏览器，版本 120 或更新（Chrome、Chromium、Edge、Brave、Arc、Vivaldi、Opera、Yandex Browser）。
- 约 50 MB 的可用磁盘空间，用于助手和 sing-box。
...
安装程序会把 noctis-host 和 sing-box 下载到你的用户数据目录，并为每个受支持的浏览器写入 native-messaging 清单。

扩展第一次与助手通信时，你的浏览器可能会显示一次性的 native-messaging 提示——请批准它。

### 首次运行

打开扩展的弹窗，粘贴一条 `vless://`、`ss://` 或 `trojan://` 分享链接（或一个订阅 URL），然后切换活动服务器。一旦 sing-box 开始接受流量，状态徽标就会变绿。
...
支持。VLESS Reality 是 sing-box 的一种标准出站，Noctis 会把 Reality 参数（Server Name、Fingerprint、SNI、Dest、public key、short ID）原样传给助手。粘贴一条 `vless://...flow=xtls-rprx-vision&security=reality` 分享链接，扩展会导入其中每个字段。

**Noctis 支持哪些代理协议？**
VLESS、VMess、Trojan、Shadowsocks、Hysteria2、TUIC、WireGuard、AnyTLS 和 ShadowTLS——凡是 sing-box 作为出站支持的都行。V2Ray 和 Xray 的分享链接可直接使用。
...
- **[sing-box](https://github.com/SagerNet/sing-box)** — 负责所有上游路由和加密的代理引擎。Noctis 是控制层；真正的工作由 sing-box 完成。
- **[V2Ray](https://github.com/v2fly/v2ray-core)** 和 **[Xray](https://github.com/XTLS/Xray-core)** — 上游的协议设计（VLESS、VMess、Reality），Noctis 通过 sing-box 兼容的出站来使用它们。
